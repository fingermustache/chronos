...
func TestJSONBValue_EncodesMap(t *testing.T) {
	...
	if decoded["url"] != "https://example.com" {
		t.Fatalf("decoded[url] = %v, want %v", decoded["url"], "https://example.com")
	}
	if decoded["method"] != "GET" {
		t.Fatalf("decoded[method] = %v, want %v", decoded["method"], "GET")
	}
	if decoded["retry"] != float64(3) {
		t.Fatalf("decoded[retry] = %v, want %v", decoded["retry"], float64(3))
	}
}
...
func TestJSONBScan_NilSetsEmptyMap(t *testing.T) {
	...
	if len(j) != 0 {
		t.Fatalf("Scan(nil) len = %d, want 0", len(j))
	}
}
...
func TestJSONBScan_FromBytes(t *testing.T) {
	...
	if j["service"] != "Worker" {
		t.Fatalf("j[service] = %v, want %v", j["service"], "Worker")
	}
	if j["method"] != "Run" {
		t.Fatalf("j[method] = %v, want %v", j["method"], "Run")
	}
}
...
func TestJSONBScan_FromString(t *testing.T) {
	...
	if j["seconds"] != float64(3600) {
		t.Fatalf("j[seconds] = %v, want %v", j["seconds"], float64(3600))
	}
}
